...
interface ServerMessage {
  type: 'CREATE_ROOM' | 'JOIN_ROOM' | 'MAKE_MOVE' | 'GAME_STATE_UPDATE' | 'UPDATE_SCORE' | 'DESTROY_ROOM';
  payload: any;
}
  ...
  private cleanupInactiveRooms() {
    ...
    for (const [roomId, room] of this.rooms.entries()) {
      if (now - room.lastActivity > this.CLEANUP_INTERVAL) {
        ...
        cleanedCount++;
        this.log(`Cleaned up inactive room`, { roomId, lastActivity: new Date(room.lastActivity).toISOString() });
      }
    }
    this.log(`Cleanup complete`, { cleanedCount, remainingRooms: this.rooms.size });
  }

  private checkForWin(board: (string | null)[][], row: number, col: number, player: string): boolean {
    const directions = [
      [0, 1],  // horizontal
      [1, 0],  // vertical
      [1, 1],  // diagonal right
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'MAKE_MOVE': {
          ...
          if (room && room.gameState.playerTurn === player && !room.gameState.winner) {
            // Update the board
            ...
            const newGameState: GameState = {
              ...
              player1Score: isWin && player === "PLAYER 1" ? room.gameState.player1Score + 1 : room.gameState.player1Score,
              player2Score: isWin && player === "PLAYER 2" ? room.gameState.player2Score + 1 : room.gameState.player2Score,
            ...
            this.log(`Invalid move`, {
              reason: !room ? "Room not found" :
                room.gameState.winner ? "Game already won" :
                  "Not player's turn"
            });
          }
          break;
